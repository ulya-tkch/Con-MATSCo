...
File containing the CoLight Agent class
"""
...
class CoLightAgent(Agent):
    ...
    def __init__(self, dic_agent_conf=None, dic_traffic_env_conf=None, dic_path=None, cnt_round=None, best_round=None,
        ...
        "MIN_EPSILON": 0.2,
        """
        if os.path.exists(os.path.join(self.dic_path["PATH_TO_MODEL"],
    ...
    def compute_len_feature(self):
        ...
        for feature_name in self.dic_traffic_env_conf["LIST_STATE_FEATURE"]:
            ...
            if "adjacency" in feature_name:
                ...
                len_feature += (self.dic_traffic_env_conf["DIC_FEATURE_DIM"]["D_"+feature_name.upper()][0] *
                                self.num_lanes,)
    ...
    def MLP(self, in_0, layers=[128, 128]):
        ...
        -output: [batch,#agents,128]
        """

        for layer_index, layer_size in enumerate(layers):
            if layer_index == 0:
                h = Dense(layer_size, activation='relu', kernel_initializer='random_normal', name='Dense_embed_%d' %
                                                                                                  layer_index)(in_0)
            else:
                h = Dense(layer_size, activation='relu', kernel_initializer='random_normal', name='Dense_embed_%d' %
                                                                                                  layer_index)(h)
    ...
    def MultiHeadsAttModel(self, In_agent, In_neighbor, l=5, d=128, dv=16, dout=128, nv=8, suffix=-1):
        ...
        print("Multi Head Attention: neighbor_repr.shape", neighbor_repr.shape)
    ...
    def build_network(self, MLP_layers=[32,32], Output_layers=[]):
        ...
        for CNN_layer_index, CNN_layer_size in enumerate(CNN_layers):
            ...
            if CNN_layer_index == 0:
                h, att_record = self.MultiHeadsAttModel(feature, In[1], l=self.num_neighbors, d=MLP_layers[-1],
                                                        dv=CNN_layer_size[0], dout=CNN_layer_size[1],
                                                        nv=CNN_heads[CNN_layer_index], suffix=CNN_layer_index)
            else:
                h, att_record = self.MultiHeadsAttModel(h, In[1], l=self.num_neighbors, d=MLP_layers[-1],
                                                        dv=CNN_layer_size[0], dout=CNN_layer_size[1],
                                                        nv=CNN_heads[CNN_layer_index], suffix=CNN_layer_index)
        ...
        att_record_all_layers = Reshape((len(CNN_layers), self.num_agents, CNN_heads[-1],
                                         self.num_neighbors))(att_record_all_layers)
        ...
        print('CoLight Agent: build_Input_end_time：', input_end_time-start_time)
        print('CoLight Agent: embedding_time:', embedding_end_time-input_end_time)
        print('CoLight Agent: total time:', network_end-start_time)
    ...
    def build_network_from_copy(self, network_copy):
        ...
        if self.att_regulatization:
            network.compile(
                optimizer=RMSprop(lr=self.dic_agent_conf["LEARNING_RATE"]),
                loss=[self.dic_agent_conf["LOSS_FUNCTION"] for _ in range(self.num_agents)] +
    ...
    def prepare_Xs_Y(self, memory, dic_exp_conf):
        ...
        _, _, target_q_values, _ = self.action_att_predict(_next_state, total_features=_next_features,
                                                           total_adjs=_next_adjs, bar=True)
    ...
    def train_network(self, dic_exp_conf):
        ...
        early_stopping = EarlyStopping(monitor='val_loss', patience=self.dic_agent_conf["PATIENCE"], verbose=0,
                                       mode='min')

        _ = self.q_network.fit(self.Xs, self.Y_total, batch_size=batch_size, epochs=epochs, shuffle=False,
    ...
    def action_att_predict(self, state, total_features=[], total_adjs=[], bar=False):
        ...
        selection = np.random.choice([0, 1], size=batch_size*self.num_agents,
                                     p=[1-self.dic_agent_conf["EPSILON"], self.dic_agent_conf["EPSILON"]])

        act = possible_action.reshape((batch_size*self.num_agents, 2))[np.arange(batch_size * self.num_agents),
                                                                       selection]
