...
File to run the generate training samples for CoLight
"""
...
class Generator:
    ...
    def __init__(self, cnt_round, cnt_gen, dic_path, dic_exp_conf, dic_agent_conf, dic_traffic_env_conf,
        ...
        print("Generator: Create intersection agent time: ", time.time()-start_time)
    ...
    def generate(self):
        ...
        while not done and step_num < int(self.dic_exp_conf["RUN_COUNTS"] /
                                          self.dic_traffic_env_conf["MIN_ACTION_TIME"]):
            action_list = []

            for i in range(self.dic_traffic_env_conf["NUM_AGENTS"]):

                if self.dic_exp_conf["MODEL_NAME"] in ["CoLight"]:
            ...
            if step_num % 1000 == 0:
                ...
                print("Generator: time: {0}, running_time: {1}".format(self.env.get_current_time() -
                                                                       self.dic_traffic_env_conf["MIN_ACTION_TIME"],
        ...
        print("Generator: start logging")
        self.env.bulk_log_multi_process()
        log_time = time.time() - log_start_time

        self.env.end_sumo()
        print("Generator: reset_env_time: ", reset_env_time)
        print("Generator: running_time: ", running_time)
        print("Generator: log_time: ", log_time)
