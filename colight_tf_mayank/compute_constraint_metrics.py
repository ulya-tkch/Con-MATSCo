...
def compute_average_max_distance(base_dir, num_inter):
    ...
    for inter_idx in range(num_inter):
        ...
        for idx in range(num_ts):
            ...
            lane_wait = curr_state["lane_wait"]
            ...
            distances_overall.append(max_distance)
    ...
    print("metric computation completed in time", datetime.now() - t_start)
    print("Average max distance", np.round(np.mean(np.array(distances_overall)), 3))
